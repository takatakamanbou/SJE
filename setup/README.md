...
**【注意】ここに記す手順で macOS に Python をインストールした環境では `pip` の代わりに `pip3` を使います．以下読みかえてね．**
...
以下のリンクをクリックしてブラウザで開く．その画面の右の方の 「**Raw**」と書かれたボタンを右クリック（または，Rawボタンの右の方にあるダウンロードアイコンを左クリック）して「リンク先を別名で保存」．`ex00.ipynb` というファイルがダウンロードされるはず．`SJE`ディレクトリへ移動しておきましょう．
